...
from resources.extensions import validPosterExtensions, tmdbApiKey
...
class Metadata:
    ...
    def writeTags(self, path, converter, artwork=True, thumbnail=False, width=None, height=None):
        ...
        if self.original:
            video["\xa9too"] = "MMT:" + os.path.basename(self.original)
        else:
            video["\xa9too"] = "MMT:" + os.path.basename(path)
    ...
    def getArtwork(self, path, thumbnail=False):
        ...
        for e in validPosterExtensions:
        ...
        if not poster:
            ...
            for e in validPosterExtensions:
                path = os.path.join(d, "mmtposter" + os.extsep + e)
                if (os.path.exists(path)):
                    poster = path
                    self.log.info("Local artwork detected, using %s." % path)
                    break
